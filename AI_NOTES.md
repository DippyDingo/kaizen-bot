...
- Added button-only Telegram flow inside `��������`:
  - `?? ���������`
  - energy step `1..5`
  - stress step `1..5`
...
- Statistics now include a dedicated `���������` block with entries, averages, active days, best energy day, and highest stress day.
...
- Improved the wellbeing question screens inside `��������` so the prompt itself is visually highlighted, not only the save confirmation.
- `wellbeing_energy` now renders a dedicated block with `?? ������ ���������` and `��� 1/2 � �������`.
- `wellbeing_stress` now renders a dedicated block with `?? ������ ���������`, `��� 2/2 � ������`, and the previously chosen energy value.
## 2026-03-07 API / Web App Foundation

- Added a new read-only `aiohttp` API layer under `backend/api/`.
- Added separate API runtime entrypoint: `python -m backend.api.main`.
- Added settings for API runtime in `bot/config.py`:
  - `api_host`
  - `api_port`
- Added Telegram Web App auth validation via `X-Telegram-Init-Data` header.
- Validation checks the signed `initData` payload against `BOT_TOKEN` and extracts `telegram_id` from the embedded Telegram user payload.
- No dev fallback via query params was added.
- Added read-only endpoints:
  - `GET /api/v1/dashboard`
  - `GET /api/v1/health`
  - `GET /api/v1/stats`
- Added manual serializer/adapter layer so API returns structured JSON without Telegram-specific HTML, emoji, or text formatting.
- API reuses the existing async SQLAlchemy service layer and does not add a second source of business logic.
- Added tests for:
  - `initData` auth validation,
  - date and stats-period parsing,
  - serializer output,
  - HTTP `401/400/404/200` behavior for API endpoints.
- Validation after the API foundation pass:
  - `python -m compileall bot backend alembic tests` passed,
  - `.\.venv\Scripts\python.exe -m unittest discover -s tests -v` passed with 52 tests.
## 2026-03-07 First Read-Only Web App Screen

- Extended the `aiohttp` app to serve a first static Web App screen from `/webapp`.
- Added static asset routes for:
  - `/webapp`
  - `/webapp/`
  - `/webapp/css/*`
  - `/webapp/js/*`
- Added the first read-only Web App UI files:
  - `webapp/index.html`
  - `webapp/css/app.css`
  - `webapp/js/app.js`
- The screen reads:
  - `/api/v1/dashboard`
  - `/api/v1/health`
  - `/api/v1/stats?period=7d`
- The screen uses Telegram Web App `initData` and does not introduce any dev auth bypass.
- Added a regression test that `/webapp` is served without API auth.
- Manual smoke-check against the live local API succeeded with a valid `initData` built for an existing Telegram user from the database:
